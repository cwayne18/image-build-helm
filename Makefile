...
.PHONY: image-scan
image-scan:
	trivy --severity $(SEVERITIES) --no-progress --ignore-unfixed $(REPO)/hardened-helm:$(TAG)
...
ifdef IID_FILE
	...
	docker buildx imagetools inspect --format "{{json .Manifest}}" $(REPO)/hardened-helm:$(TAG) | jq -r '.digest' > "$(IID_FILE)"
endif
